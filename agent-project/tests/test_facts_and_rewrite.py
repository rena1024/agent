...
def test_rewrite_query_happy_path():
    ...
    q = rewrite_query(llm=llm, question="根据内部资料，说说四次挥手", facts_block="", max_chars=30, trace_id="t")
    assert "四次挥手" in q


def test_rewrite_query_bad_json_falls_back():
    llm = StubLLM("not-json")
    q = rewrite_query(llm=llm, question="hello world", facts_block="", max_chars=10, trace_id="t")
    assert q == "hello worl"
